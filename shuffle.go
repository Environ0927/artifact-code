...
func msgSim_rain(msgType, msgBlocks int, numServers int) []byte {
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(startI, endI, threadNum int) {
			for msgCount := startI; msgCount < endI; msgCount++ {
				clientTransmission := msgSim_rain(msgCount%26, msgBlocks, numServers)
...
func exp_rain(vec_dim, batchsz int) {
	d := vec_dim
	...
	hdShares1 := recvFromOtherParty()            
	hdMerged := MergeShares(hdShares0, hdShares1)  
